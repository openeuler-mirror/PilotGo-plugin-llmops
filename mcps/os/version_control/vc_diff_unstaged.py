...
def git_diff_unstaged(directory=None, files=None):
    """
    查看 Git 仓库中未暂存的变更
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "成功获取未暂存的变更" if has_changes else "没有未暂存的变更",
            "directory": target_dir,
            "files": files,
        ...
        return {
            "status": "error",
            "message": f"获取未暂存变更失败: {e.stderr.strip()}",
            "directory": target_dir,
            "files": files,
            "diff": "",
            "output": "",
...
TOOL_CONFIG = {
    "name": "vc_diff_unstaged",
    "function": git_diff_unstaged,
    "description": "查看Git仓库中未暂存的变更",
    "parameters": {
        "type": "object",
        "properties": {
            "directory": {
